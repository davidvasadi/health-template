...
export function VouchersFlow({
    ...
    const steps: Array<{ kicker?: string; title?: string; key?: StepKey }> = page?.steps?.length
        ? page.steps
        : [
            { kicker: "VÁLASSZON", title: "Utalványtípus", key: "select" },
            { kicker: "ADJA MEG", title: "Személyre szabás", key: "personalize" },
            { kicker: "KIZÁRÓLAG", title: "Banki átutalás", key: "transfer" },
        ];
    ...
    async function createOrder() {
        ...
        try {
            ...
            if (!base) throw new Error("NEXT_PUBLIC_API_URL nincs beállítva.");

            // ✅ Strapi v5 relation connect
            const res = await fetch(`${base}/api/voucher-orders`, {
                method: "POST",
                ...
                body: JSON.stringify({
                    data: {
                        ...
                        email,
                        message,
                        voucher: selectedVoucher!.documentId ?? selectedVoucher!.id,
                    },
                }),
                cache: "no-store",
            });
            ...
            if (!res.ok) {
                ...
                throw new Error(`Order create hiba (${res.status}). ${t}`);
            }

            const created = await res.json();
const createdKey = (created?.data?.documentId ?? created?.data?.id ?? created?.documentId ?? created?.id) as string | number | undefined;
            ...
            const res2 = await fetch(`${base}/api/voucher-orders/${createdKey}?populate=voucher`, { cache: "no-store" });
            if (!res2.ok) {
                setOrder(created?.data ?? created);
                go("transfer");
                scrollToTop();
    ...
    const referenceCode =
        order?.attributes?.reference_code ?? order?.reference_code ?? order?.attributes?.referenceCode ?? "";
    ...
    const iconBubble = "grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#057C80]/10 text-[#057C80]";

    return (
        <div ref={flowTopRef} className="w-full scroll-mt-24">
            {/* HERO */}
            <div className="mx-auto  text-left mb-10 md:mb-16">
                <h1 className="text-3xl md:text-7xl font-semibold tracking-tight text-neutral-950">
                    {page?.heading ?? t(locale, "heroFallback")}
                </h1>
                {page?.sub_heading ? (
                    <p className="text-left mt-4 max-w-2xl text-sm md:text-lg text-neutral-500">{page.sub_heading}</p>
                ) : null}
            </div>
                                    ...
                                    return (
                                        <div key={s.key ?? i} className="flex items-start gap-3">
                                            ...
                                            <div className="leading-tight pt-1">
                                                <div className="text-[11px] font-semibold tracking-wide text-slate-400">{s.kicker}</div>
                                                <div className={cn("text-sm font-semibold", active ? "text-slate-900" : "text-slate-500")}>
                                                    {s.title}
                                                </div>
                                            </div>
                                        </div>
...
<div className="hidden md:block">
  <div className="flex items-center justify-center gap-10">
    {steps.slice(0, 3).map((s, i) => {
      ...
      const isBankStep = i === 2; // 3. lépés: bank ikon
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {done ? (
                ...
                <Landmark className={cn("h-5 w-5", done || active ? "text-white" : "text-[#057C80]")} />
              ) : (
                i + 1
              )}
            </div>
                        ...
                        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                            ...
                            <div className="rounded-3xl border border-slate-200 bg-white p-6">
                                ...
                                <div className="mb-5 flex items-center gap-3">
                                    ...
                                    <div className="text-sm font-semibold text-slate-900">
                                        {page?.voucher_picker_title ?? "Válassza ki az utalványt"}
                                    </div>
                                </div>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="mt-3">
                                                            ...
                                                            {v.sub_heading ? <div className="mt-1 text-xs text-slate-600">{v.sub_heading}</div> : null}
                                                            {v.price ? (
                                                                <div className="mt-3 text-base font-semibold text-[#057C80]">{formatHUF(v.price)}</div>
                                                            ) : null}
                                                        </div>
                            ...
                            <div
                                ...
                                className={cn(
                                    ...
                                    rightPanelSelected ? "border-[#057C80] ring-2 ring-[#057C80]/15" : "border-slate-200"
                                )}
                            >
                                <div className="mb-5 flex items-center gap-3">
                                    <span className={iconBubble}>
                                    ...
                                    <div className="text-sm font-semibold text-slate-900">
                                        {page?.personalization_title ?? "Személyre szabás"}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-[11px] font-semibold tracking-wide text-slate-500">
                                            {page?.recipient_name_label ?? "AJÁNDÉKOZOTT NEVE"}
                                        </label>
                                        <input
                                            value={recipientName}
                                            onChange={(e) => setRecipientName(e.target.value)}
                                            placeholder={page?.recipient_name_placeholder ?? "Pl: Kovács Anna"}
                                    ...
                                    <div>
                                        <label className="block text-[11px] font-semibold tracking-wide text-slate-500">EMAIL</label>
                                        <input
                                            value={email}
                                            onChange={(e) => setEmail(e.target.value)}
                                            placeholder="pl. anna@gmail.com"
                                            disabled={!selectedVoucher}
                                            className={cn(
                                                "mt-2 w-full rounded-xl border px-4 py-3 text-sm outline-none",
                                                "border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-400 focus:border-[#057C80]/40",
                                    ...
                                    <div>
                                        <label className="block text-[11px] font-semibold tracking-wide text-slate-500">
                                            {page?.message_label ?? "SZEMÉLYES ÜZENET"}
                                        </label>
                                        <textarea
                                            value={message}
                                            onChange={(e) => setMessage(e.target.value)}
                                            placeholder={page?.message_placeholder ?? "Írjon egy kedves üzenetet az utalványra..."}
                                            disabled={!selectedVoucher}
                                            className={cn(
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            !canSubmit() || busy ? "bg-slate-200 text-slate-500" : "bg-[#057C80] text-white hover:bg-[#00969e]"
                                        )}
                                    >
                                        {busy ? "Feldolgozás..." : buyBtn.text}
                                        <ArrowRight className="h-4 w-4" />
                                    </button>

                                    {page?.bank_transfer?.warning_text ? (
  <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs md:text-[11px] font-semibold text-amber-900">
    <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                ...
                                                {page?.help_text ? <div className="mt-1 text-sm text-slate-600">{page.help_text}</div> : null}
                                            </div>
                                        </div>
    ...
    return (
        <div className="mx-auto max-w-4xl">
            <div className="flex flex-col items-center text-center">
                ...
                <h2 className="mt-5 text-3xl md:text-5xl font-semibold tracking-tight text-slate-900">
                    {page?.success_title ?? "Köszönjük a vásárlást!"}
                </h2>
            ...
            <div className="mt-10 rounded-3xl border border-slate-200 bg-white p-6 md:p-7">
                <div className="mb-5 flex items-center gap-3">
                    ...
                    <div className="text-sm font-semibold text-slate-900">{bank?.section_title ?? "Átutalási adatok"}</div>
                </div>

                {/* ✅ NEW: stacked row-cards (mobile + desktop) */}
                <div className="mt-5 space-y-4">
                    <BankRow
                        label={bank?.beneficiary_label ?? "Kedvezményezett"}
                        value={bank?.beneficiary_name ?? ""}
                        copyValue={bank?.beneficiary_name ?? ""}
                        variant="normal"
                    />
                    <BankRow
                        label={bank?.account_label ?? "Számlaszám"}
                        value={bank?.account_number ?? ""}
                        copyValue={bank?.account_number ?? ""}
                        variant="normal"
                    />
                    <BankRow label={bank?.amount_label ?? "Összeg"} value={amountText} copyValue={amountText} variant="amount" />
                    <BankRow
                        label={bank?.reference_label ?? "Közlemény / Megjegyzés"}
                        value={referenceCode || "—"}
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <div className="mt-2 text-sm text-slate-600">
                                    {bank?.delivery_note
                                        ? bank.delivery_note
                                        : "Az utalványt (PDF formátumban), az utalás beérkezését követő 24 órán belül küldjük a megadott e-mail címre."}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-5">
                        <div className="flex items-start gap-3">
                            ...
                            <div className="w-full">
                                <div className="text-sm font-semibold text-slate-900">{page?.help_title ?? "Segítségre van szüksége?"}</div>
                ...
                <div className="mt-10 flex justify-center">
                    ...
                    >
                        ...
                        {page?.back_to_home_label ?? "Vissza a főoldalra"}
                    </a>
                </div>
            </div>
        </div>
  ...
  return (
    <div className={cn("rounded-2xl border px-4 py-4", tone)}>
      <div className="flex items-center justify-between gap-4">
        {/* MOBILE: label + value egymás alatt | DESKTOP: egy sor */}
      ...
      {/* ✅ DESKTOP ONLY: label/value egy sorban legyen (mint a screenshot desktopon) */}
      {/* Mobilon már jó a fenti stackelt megjelenés, ezért nem kell extra. */}
    </div>
  );
}
