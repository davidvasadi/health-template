...
export function VouchersFlow({
  ...
  async function createOrder() {
    ...
    try {
      if (!selectedVoucher) throw new Error("Nincs kiválasztott utalvány.");
      ...
      if (!voucherKey) throw new Error("Hiányzó voucher azonosító.");
      ...
      if (!res.ok) {
        ...
        throw new Error(`Order create hiba (${res.status}). ${txt}`);
      }
  ...
  return (
    <div ref={flowTopRef} className="w-full scroll-mt-24">
      ...
      <div className="mx-auto text-left mb-10 md:mb-16">
        ...
        {page?.sub_heading ? (
          <p className="text-left mt-4 max-w-2xl text-sm md:text-lg text-neutral-500">
            {page.sub_heading}
          </p>
        ) : null}
      </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="w-full">
                <div className="text-sm font-semibold text-slate-900">
                  {page?.help_title ?? "Segítségre van szüksége?"}
                </div>
