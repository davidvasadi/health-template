...
import { IconChevronRight, IconSparkles, IconMessageDots } from "@tabler/icons-react";
  ...
  return (
    ...
    >
      ...
      <div className="text-left">
        <p className="text-white/60 text-[10px] font-semibold uppercase tracking-wide mb-1">{t.collection}</p>
    ...
    <div className={cn(bentoItem, "p-8 flex flex-col justify-between bg-[#f8fafc] hidden md:flex lg:row-span-2", className)}>
      <div>
        ...
        <div className="space-y-6">
          <div>
            <p className="text-[10px] font-semibold text-[#057C80] uppercase tracking-wide mb-3">{t.filterBy}</p>
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                ...
                <span className="text-neutral-400 tabular-nums">{counts.easyCount}</span>
              </button>
              ...
              >
                ...
                <span className="text-neutral-400 tabular-nums">{counts.midCount}</span>
              </button>
              ...
              >
                ...
                <span className="text-neutral-400 tabular-nums">{counts.hardCount}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="bg-[rgba(0,121,128,0.06)] p-5 rounded-2xl border border-[rgba(0,121,128,0.12)]">
        <p className="text-[11px] font-semibold text-[#057C80] leading-snug">{t.difficultyTip}</p>
      </div>
    </div>
  );
}

export function ContactTile({ t, href, className }: { t: TDict; href: string; className?: string }) {
  return (
    <div className={cn(bentoItem, "p-8 bg-[#f8fafc] border-dashed border-2 border-neutral-200 flex flex-col justify-center", className)}>
      ...
      <h4 className="text-lg font-semibold tracking-tight mb-2 text-neutral-950">{t.askTitle}</h4>
      <p className="text-xs text-neutral-500 font-medium leading-relaxed mb-4">{t.askDesc}</p>
      <Link href={href as never} className={cn("text-xs font-semibold text-[#057C80] uppercase tracking-wide hover:underline", focusRing)}>
        {t.askCta}
      </Link>
    </div>
  );
  ...
  return (
    ...
    >
      <div className="relative z-10 text-left">
        <p className="text-white/60 text-[10px] font-semibold uppercase tracking-wide mb-2">{t.expertPick}</p>
        ...
        <div className="mt-4 flex gap-2">
          <span className="px-2 py-0.5 bg-white/10 rounded text-[9px] font-semibold">{t.tagAllLevels}</span>
          <span className="px-2 py-0.5 bg-white/10 rounded text-[9px] font-semibold">{t.tagMinutes}</span>
        </div>
      </div>

      <div className="relative z-10 flex flex-col items-end text-right">
        <span className="text-5xl font-semibold tracking-tight mb-1">{count >= 24 ? "24+" : String(count)}</span>
        <span className="text-[10px] uppercase font-semibold tracking-wide opacity-60">{t.activeVideos}</span>
      </div>
