...
function emitOverlayNavigate(kind: OverlayKind, href?: string | null) {
    ...
    window.dispatchEvent(
        new CustomEvent(ROUTE_OVERLAY_NAV_EVENT, { detail: { kind, href: href ?? null } })
    );
}
...
function shellRevealAnimated(reduce: boolean, ms: number, pullPx: number) {
    ...
    el.style.willChange = "opacity, transform";
    el.style.transition = `opacity ${ms}ms cubic-bezier(${EASE.join(
        ","
    )}), transform ${ms}ms cubic-bezier(${EASE.join(",")})`;
...
export default function RouteOverlay() {
    ...
    const panelTransition =
        ui.status === "enter"
            ? { duration: panelEnterMs / 1000, ease: EASE, delay: reduce ? 0 : 0.12 }
            : { duration: panelExitMs / 1000, ease: EASE };

    const showWipe = !reduce && ui.status === "enter" && (ui.kind === "first" || ui.kind === "route");
    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* ✅ BRAND WIPE (teal sweep) — a fehér fölött, a content mögött */}
                {showWipe && (
                    <motion.div
                        key={`wipe-${ui.logoKey}`}
                        className="absolute inset-0 bg-gradient-to-b from-transparent via-[#057C80]/18 to-transparent backdrop-blur-[1px]"
                        initial={{ y: "-110svh" }}
                        animate={{ y: ["-110svh", "0svh", "110svh"] }}
                        transition={{
                            duration: 0.9,
                            ease: EASE,
                            times: [0, 0.38, 1],
            ...
            >
                <div className="absolute inset-0 grid place-items-center px-6 pointer-events-none">
                    <motion.div
                        className={["pointer-events-none select-none", "relative"].join(" ")}
                        initial="exit"
                    ...
                    >
                        <Preload key={ui.logoKey} tagline="The Place Studio" />
                    </motion.div>
                </div>
            </motion.div>
        </div>
